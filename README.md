...
Для полноценной рассылки всем игрокам user-бота добавь ещё и:

```bash
USER_BOT_TOKEN=...
```

Это не делает admin service пользовательским ботом. Этот токен нужен только чтобы админка могла отправлять сообщения игрокам через их основной bot chat.
...
- Для рассылки всем игрокам из админки admin service должен знать `USER_BOT_TOKEN`, потому что Telegram разрешает писать пользователю только тому боту, которого этот пользователь уже запускал.
- Топ-10 лидеров в админ-боте берётся из общей Postgres-базы.
- Игроки из основной базы получают рассылку через user-бота.
- Те, кто нажимал `/start` только в админ-боте, получают рассылку через admin-бота.
